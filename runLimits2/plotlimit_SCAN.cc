...
double return_limit(TString dir, int x, int y, double BDTcutoffset){
   ...
   if ( (dir == "T2tt") && ( BDTcutoffset == 0.2) )    {dataset_name = "T2tt_CUT0.2";  }
  		  ...
  		  char filename[500];
                  ...
                  sprintf(filename,"/afs/cern.ch/work/s/sigamani/public/CMSSW_6_1_1/src/HiggsAnalysis/CombinedLimit/LimitsBDT_6_mT100/%s/ASYMPTOTIC_CLS_RESULT_S%d-N%d.root", dataset_name,x, y);
...
void plot_limit(TString decay_mode){
   ...
   TH2D *SMS2 = new TH2D(decay_mode+"SRs","",26,162.5, 812.5, 16, 12.5,412.5);
	
              //for(int x=175; x<800; x+=25){
              for(int x=175; x<725; x+=25){
                      ...
                      for(int y=25; y<=300; y+=25){
						...
						if ( (decay_mode == "T2tt")  || (decay_mode == "T2bw025")) {
						...
						if (mvaval+1 == 5) offset = -0.15;	
						if (mvaval+1 == 6) offset = -0.1;	
						if (mvaval+1 == 7) offset = -0.05;	
						if (mvaval+1 == 8) offset = 0.0;	
						if (mvaval+1 == 9) offset = 0.05;	
						if (mvaval+1 == 10) offset = 0.1;	
						if (mvaval+1 == 11) offset = 0.15;	
						if (mvaval+1 == 12) offset = 0.2;	
	...
	gStyle->SetOptStat(0);
           ...
           SMS3->GetXaxis()->SetTitle("Mass STOP");
           SMS3->GetYaxis()->SetTitle("Mass LSP");
	   //SMS3->GetZaxis()->SetTitle("Optimal BDT cut off-set"); 
	   SMS3->GetZaxis()->SetTitle("Optimal BDT cut"); 
...
//	   SMS->GetZaxis()->SetRangeUser(1,12);
	   ...
	   key->AddText("4: BDT cut - 0.35");
	   key->AddText("5: BDT cut - 0.30");
	   key->AddText("6: BDT cut - 0.25");
	   key->AddText("7: BDT cut - 0.05");
	   key->AddText("8: BDT cut + 0.00");
	   key->AddText("9: BDT cut + 0.05");
	   key->AddText("10: BDT cut + 0.10");
	   key->AddText("11: BDT cut + 0.15");
	   key->AddText("12: BDT cut + 0.20");
           ...
           SMS3->SetMarkerSize(1.);
           SMS3->SetMarkerColor(kWhite);
           SMS3->Draw("COLZ TEXT");

  	   key->Draw("same");
           c1.RedrawAxis();
           c1.SaveAs("~/www/"+TString(decay_mode)+".png");
